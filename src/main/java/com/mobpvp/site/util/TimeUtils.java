...
public class TimeUtils {
    ...
    public static String formatCalendarString(long input, boolean time) {
        return time ? DATE_FORMAT_2.format(input) : DATE_FORMAT.format(input);
    }

    public static long parseTime(String input) {
        if (input.equals("0") || input.equals("")
                || input.equalsIgnoreCase("0s"))
            return 0;
    ...
    public static String formatHHMMSS(long input, boolean displayMillis, TimeUnit timeUnit) {
        ...
        StringBuilder result = TimeUtils.MM_SS_BUILDER.get();
        result.setLength(0);
    ...
    public static String formatDate(long input, boolean time, TimeZone timeZone) {
        ...
        DateFormat formatter = new SimpleDateFormat("dd/MM/yyyy" + (time ? " hh:mm:ss a z" : ""));
